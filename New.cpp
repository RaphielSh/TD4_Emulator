...
std::vector<uint8_t> getProgram(const std::string& path) { 
	...
	std::vector<uint8_t> binProg; //init vector to store program content
	
	char byte;	//init 1 byte for reading
	while (file.read(&byte, 1)) { binProg.push_back(static_cast<uint8_t>(byte)); }
	return binProg;
}
...
void executeInstruction(Registers& cpu, const std::vector<uint8_t>& program) {
	...
	switch (opcode) {
		...
		case 0x1: // MOV B, im
			cpu.A = operand;
			std::cout << "MOV B, " << std::bitset<4>(operand) << "\n";
			break;
		case 0x2: // ADD A, B
		...
		case 0x4: // JMC im
		    if (!cpu.carry){
		        ...
		        return;
		    } else { std::cout << "JMC " << std::bitset<4>(operand) << "(перенос есть, нет перехода)\n"; }
		    break;
		case 0x5: // IN A
		    cpu.A = cpu.IN;
		    std::cout << "IN A\n";
